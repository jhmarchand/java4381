    ...
    public void loop() {
        ...
        if (runtime.seconds() <0.5) {
            ...
            if (robot.seeBlue(true) == false || Blue == true) {
                robot.DriveMecanum(0, driveSpeed, 0.09);
               extraStrafe = 0.5;
                Blue = true;
            }
            else {
        ...
        else if (runtime.seconds() < 6.0 + backwards + extraStrafe) {
            driveSpeed = 0.5;// add strafing if it went forwardmaybe .5?
            robot.DriveMecanum(driveSpeed*-1, 0, 0);
        }
        else if (runtime.seconds() < 6.2 + backwards + extraStrafe){
            robot.DriveMecanum(0,driveSpeed,0);
        }
        else if (runtime.seconds() < 7.2 + backwards + extraStrafe){
            robot.partial();
            robot.DriveMecanum(0,0,0);
        }
